...
- **Underwriting at a premium** shows as a *positive* worst in the `EUSD` par window: the pool is
  paying above par, and the liquidation discount has to earn that back before anything is profit.
  The sign is the whole signal — read it, do not read the magnitude alone.
