...
"Stake everything at block 0" is deliberately not the answer: the yield moves, the withdrawal queue
congests with size and with other people's queue position, and a slash can cut the redemption rate.
If the strategy has parked everything and stopped thinking, that is worth fixing.
...
- **The queue is the risk the trade aggregate cannot see.** A settled-trade figure that looks fine
  while the marked value falls is usually an exit that will not finalize before the run ends —
  scored at what the pool would pay, not at par. `estimatedQueueDelayBlocks` and
  `blocksRemaining` in the latest observation are the pair to read together.
- **`lst:market-vs-redemption` in the discounts section is your window.** It is `discountBps` over
  the whole interval, with the widest point and the block it happened on, and whether it is still
  open. Buying the LST in the pool and redeeming at par is a different trade from staking, with a
  different risk, and it is only available while that window is open.
- **Chasing the yield** shows as a strategy that was tuned to a `yieldPerBlockBps` the run has since
  resampled. That is a real revision; a slash is not.
