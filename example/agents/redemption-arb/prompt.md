...
- **`EUSD` in the stables section is the whole subject.** It reports the interval's departures from
  par as windows with a widest point and a block; negative is below a dollar, which is the
  redemption discount. That is the answer to "did the discount open and did the agent take it": a
  window with no decisions inside it is a margin that is too wide, and a window the agent traded
  into and lost on is an exit cost that is understated.
