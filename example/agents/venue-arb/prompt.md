...
- The gap series to read is `uniswap:WETH`, `balancer:WETH` and `curve:WETH`. The *cross-venue*
  edge is the difference between two of them, not either one's gap against fair — a block where
  every venue is 30 bps rich is a fair-price move, not an arbitrage.
- Where a venue quotes one, the digest appends `round trip costs N bps here`. An edge below that
  number is fee bleed by construction, whatever the gap looks like.
- A direction the wallet cannot fund shows up as `rejected (swap)`, not as a missed trade. If those
  dominate the interval, the fix is the funding check, not the threshold.
