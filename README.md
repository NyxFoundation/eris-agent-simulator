...
ターミナル 2（初回のみ `build:contracts` で PriceFeed + モックを compile。run 設定は `eris.config.yaml`。スモークなら `ERIS_RUN_BLOCKS` を小さく / `ENABLED_PROTOCOLS: [uniswap]`）:
...
# 別ファイルを使う場合: npm run sim:realtime -- --config eris.config.full.yaml
```
...
- `eris.config.yaml` が無ければ committed の `eris.config.example.yaml` が既定として使われる（env からの設定読取は廃止）。一回限りの上書きは CLI フラグ（`--seed` / `--blocks` / `--protocols` / `--agents` 等）。
