...
競技全体
 └ エポック  ×k        1 つの世界。全参加単位が同一の初期条件から同時に始まり、
    ...
    │
    ├ 評価グループ      同時に走る参加単位の集まり（上限 32）。エポックごとに再抽選される
    │
...
初期資本は全参加単位に同じものが配られます: **8 WETH + 0.4 WBTC + 25,000 USDC**、加えてガス用の ETH。何も動かさない参照エージェント（ベンチマーク）が各グループに置かれます。

> **ここまでは本番の採点であって、このリポジトリのローカル採点ではありません。** `npm run backtest` と
> `summary.json` は今も旧方式で採点します（12 ブロック区間ごとの超過対数リターンから
> `mean − 0.25 × std` を出し、シナリオ内 z-score → レジーム等重み平均）。評価グループの分割・
> エポックごとの再抽選・後半の加重はローカルには**実装されていません**。
> **ローカルの数字は「自分の版どうしを比べる」ためのもので、本番の順位の予測ではありません。**
...
| 評価区間 | **epoch** / ダッシュボードの「ラウンド」 | `summary.json` の `epochScores`、`run.epochBlocks: 12` |
...
生成されたコードは **cheatcode 静的検査 → コンパイル**（関数式の評価に 1 秒の上限）を通ってから設置されます。**設置前に試運転はしません。** 設置後は `decide` の**呼び出しごとに 2 秒**の上限がかかり、超えるとそのラウンドが `decide error:` として記録されます（`EXECUTOR_TIMEOUT_MS`）。落ちた改訂は設置されず、記録が残って戦略は無改変で走り続けます。自動 rollback はありません。戻すかどうかはモデルの判断です（版履歴を渡して `revertTo` で行う）。
...
練習で身につくのは執行と観測の扱いです。**エポックのリセット・評価グループ・偏差値による採点は練習には存在しません**。そこは本番だけの構造です。
