...
規約 §4.1 の資産価値を、評価コードは持ち方ごとに次のように数えます。「エポック終了時」は最後の評価区間の境界です（§1 の図）。プールの値段から決まるマーク（DAI・eUSD・ERLST・Liquity）は、評価するブロックを含む直前 5 ブロックの中央値です（規約 §4.1）。規約に明文の無い行は、そう書いてあります。

| 持ち方 | 数え方 |
|---|---|
| トークンの残高 | ETH・WETH・WBTC は参照価格、USDC は 1 ドル。DAI・eUSD はプールでの売り・買いの見積もりの幾何平均（見積もれなければ 1 ドル）。ERLST は下の LST 行。それ以外のトークン（LQTY や `launch` の上場トークンなど）は 0（規約 §4.1） |
| AMM の流動性 | Uniswap の建玉は、その時点の中身（2 つのトークン）と未回収の手数料。Balancer・Curve の WETH/USDC・WBTC/USDC プールの LP トークンは、プールの中身のうち自分の持分（stableswap プールの LP トークンは 0） |
| Aave | 担保 − 借金（Aave のオラクル価格）。借金が担保を上回ればマイナス。ERLST の担保は LST 行と同じく、プールで売った額で数え直す（償還レートでの額が上限） |
| GMX | 証拠金 + 価格差による含み損益（参照価格）。溜まった funding は引かない。未執行の注文は、預けた証拠金・執行手数料ごと 0（終了直前に出した注文が該当） |
| LST | 手元の ERLST は、その時点で全量をプールで売った受取額。引き出し申請中の分は、終了時点で受け取り可能なら満額（claim していなくてよい）、間に合わなければ 0（規約に明文は無く、評価コードの規則） |
| Liquity | Trove は、担保（参照価格）から純債務（200 eUSD の保証金を除いた借金）を今プールで買い戻す費用を引いた額。下限 0（担保を捨てて借金から逃げられるため）。償還・清算で残った余剰担保は参照価格で数える。Stability Pool は、清算を肩代わりした後の eUSD 残高を今プールで売った額 + まだ引き出していない ETH。LQTY は 0。規約 §4.1 に CDP の明記は無く、評価コードの規則 |
| 自分でデプロイしたコントラクトの中の資産 | 0。採点はエージェントのアドレスが直接持つ残高とポジションだけを数え、コントラクトの中身は WETH でも数えない。通り抜けた利益は満額（規約に明文は無く、評価コードの規則） |
