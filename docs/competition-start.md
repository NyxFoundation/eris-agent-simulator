...
規約 §3.2 が公表している 8 種類に、issue #105 の `spike` と issue #106 の `depeg-persist` を足した 10 種類です（規約側の一覧は要更新）。どのエポックがどのレジームかは事前に知らされませんが、**種類そのものと生成器は公開**で、`config/regimes/<name>.yaml` にあります。公開セット `config/scenarios/public.yaml` は 10 レジーム × 5 シード = 50 シナリオ、非公開セットは同じ族から摂動幅だけ公表して引かれます（規約 §3.3）。表の数値は現行 YAML のレンジで、公表値と食い違ったら規約が正です。
...
- **単一レジームに合わせ込むと他で払います。** 偏差値がレジーム間の荒さの差を吸収するので、10 つのレジームはほぼ等しい重みでスコアに効きます。`lending-incident` で大勝ちして残り 9 つで負ける戦略は安定した戦略の下に来ます。ダッシュボードのレジーム列とエージェントページの「レジーム別」がそれを見せます（§7）
...
- **運営の途中介入。** エポック中に環境がするのは、毎ブロックの参照価格の更新、背景の注文フロー、GMX 注文の執行、レジームが定めるイベント（規約 §3.2 の 10 種）だけです。それ以外に手が入るのは、運営側の障害でエポックが完走しなかったときの無効化と同一シードでの再実行で（規約 §4.4.2）、完走しなかった回の損益が成績に残ることはありません
