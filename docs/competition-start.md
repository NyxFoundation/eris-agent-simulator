...
- **単一レジームに合わせ込むと他で払います。** 偏差値がレジーム間の荒さの差を吸収するので、11 つのレジームはほぼ等しい重みでスコアに効きます。`lending-incident` で大勝ちして残り 10 で負ける戦略は安定した戦略の下に来ます。ダッシュボードのレジーム列とエージェントページの「レジーム別」がそれを見せます（§7）
...
- **運営の途中介入。** エポック中に環境がするのは、毎ブロックの参照価格の更新、背景の注文フロー、GMX 注文の執行、レジームが定めるイベント（規約 §3.2 の 11 種）だけです。それ以外に手が入るのは、運営側の障害でエポックが完走しなかったときの無効化と同一シードでの再実行で（規約 §4.4.2）、完走しなかった回の損益が成績に残ることはありません
