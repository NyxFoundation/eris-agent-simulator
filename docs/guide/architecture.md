...
- **Scoring is reconstructed after the run** (`core/src/realtime/reconstruct.ts`) — a Multicall3 keyed on blockNumber writes each agent's value series at the same cross-section into `events.jsonl`, aggregated into `runs/<id>/summary.json`. The series is cut at evaluation-interval boundaries (`valueSeries.intervalSeries`), stored alongside it, and the score's P is read off its first and last boundary ([Scoring](scoring.md)). The same historical reads also produce `market.json`, a reporting-only per-venue series that never feeds scoring.
