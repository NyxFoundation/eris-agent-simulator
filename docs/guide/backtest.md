...
- `config/regimes/crash.yaml` — trapezoidal crash, no victims: a price gap plus a `liquidityPull` on the same window, so the book thins exactly while the price moves ([#52](https://github.com/NyxFoundation/eris-agent-simulator/issues/52) phase 1, uniswap only; the two magnitudes interact and are not yet co-calibrated)
