...
`DECIDE_TIMEOUT_MS` (5,000 ms, `runtime/decideTimeout.ts`; rules §2.3), generated executors
included (`runtime/improve.ts` races them against the same constant). Past the bound the block is no
action and the late answer is dropped. But the bound is not the binding constraint:
