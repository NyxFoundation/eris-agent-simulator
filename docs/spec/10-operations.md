...
**入札は昇順に送る**ので到着順と手数料順が逆になる。到着順を保つだけのビルダーは降順プローブなら通ってしまうが、この昇順プローブでは落ちる。昇順プローブは maxFeePerGas = tip で署名するので、fee 順であることしか測らない。

続けて**キープローブ**が「どのフィールドで並べているか」を測る。毎ラウンド、正直な入札（tip = maxFeePerGas = 2 gwei）と過大入札（tip 1 gwei / maxFeePerGas 3 gwei）を到着順を交互にして送り、`max-fee` / `paid` / `arrival` / `ambiguous` を判定する（`core/src/orderingKey.ts`）。**anvil は `max-fee`**（過大入札が安く払って前に入る）で、そこでは手数料ルール（maxFeePerGas ≤ tip。ゲートウェイ・ランタイム・`postRunCheck`）が順序と支払いを一致させている。過大入札はゲートウェイが拒否する形なので、ノードに直接向けて実行する。

引数なし（`blocks.csv`）の検査は、逆転の直前の tx が `maxFeePerGasWei` では順序どおりだった場合、ビルダーではなく送信者の手数料ルール違反としてその旨を付記する。
