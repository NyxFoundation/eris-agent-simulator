...
`summary.json` にはスコアの材料（境界ごとの価値系列と P）が入り、順位は `matrix.json` からの派生物（`standings.json`）で、`computeStandings` が再計算できる（ADR 0017 §4）。採点規則そのものは規約 §4.4 で確定した（ADR 0022）。→ [06](06-scoring.md)
...
| T / Score | 競技スコア。1 エポックの P の偏差値 T = 50 + 10 (P − μ) / σ と、その加重平均（規約 §4.4、ADR 0022） |
