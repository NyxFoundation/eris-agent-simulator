...
- **本番の競技運営そのもの** — 提出・審査・賞金は [競技規約](https://ascon.dev/rules) の管轄で、本システムは実行基盤のみを提供する
...
`summary.json` にはスコアの材料（境界ごとの価値系列と P）が入り、順位は `matrix.json` からの派生物（`standings.json`）で、`computeStandings` が再計算できる（ADR 0017 §4）。採点規則そのものは規約 §4.4 で確定した（ADR 0023）。→ [06](06-scoring.md)
...
| T / Score | 競技スコア。1 エポックの P の偏差値 T = 50 + 10 (P − μ) / σ と、その加重平均（規約 §4.4、ADR 0023） |
| 破産 | 資産価値 ≤ 0。床処理も凍結も無く、負の値がそのまま P に入る（規約 §4.5） |
