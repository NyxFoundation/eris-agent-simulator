...
現行制約の解消）。market series も同様に逐次追記へ。エポック長は週スケールに再較正する
（λ の実効的厳しさは epoch 長に連動するため、採点側と一体で決める。値は未決 = 下表）。
...
| 項目 | 決めない理由 | いつ決めるか |
|------|------------|------------|
| エポック長の値（週スケール較正） | λ の実効的厳しさと連動し、採点側（#56 / ADR 0020 の再比較）と一体 | 逐次採点の実装前 |
...
| ストレス窓予定の公開度合い | 練習価値（予告あり）と本番同等性（非公開）のトレードオフ | 期間設計時 |
