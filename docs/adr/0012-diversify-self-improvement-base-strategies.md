...
- **gmx keeper**: 実走デバッグ（`ERIS_GMX_KEEPER_DEBUG=1` の可視化）で **keeper は最初から機能していたことが判明＝修正不要**。3 段階で仮説が覆った: (1) blocks.csv で keeper の executeOrder は **全件 success**（「投げっぱなしで未執行」は誤り）→ (2) OrderCancelled イベントは **無し**（「注文キャンセル」も誤り）→ (3) receipt のイベント集計で **`PositionIncrease` + `OrderExecuted` を emit**＝**position は実際に建っている**。`buildCreateOrderParams` の `receiver=owner`(agent) で position は agent に建ち、`reconstruct` も `getAccountPositions(agent)` を valueUsdc に加算する（acceptablePrice は `loose` max/0、oracle timestamp は `block.timestamp` 鮮度常時通過＝いずれも無罪）。gmxperp が複数回 open するのは **realtime の observe 遅延**（keeper execute が次ブロック→position 認識前に再 open、最終的に noop hold へ移行）で keeper の問題ではない。netPnl マイナスは perp の execution fee/gas を 20 block で回収できないだけ。→ **メモリ `gmx-realtime-keeper-no-execute` の診断は誤りで、gmx 系 base（gmxperp/gmxrev/gmxtrend/dnlp）は realtime で機能する**。診断用の可視化は `ERIS_GMX_KEEPER_DEBUG` gate で常設（通常 run に不干渉）。
