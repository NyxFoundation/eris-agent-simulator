...
- **C2（順位安定）**: 「seed 間の mean Spearman」をそのまま「**反復間の mean Spearman**」として使う（コード不変）。ノイズ増を吸収するため最低反復数 N を引き上げる。
...
| 本 ADR が ADR 0001/0002/0004 を supersede するか補完か | realtime 一本化の完了後に判断 | 一本化完了後にレビュー |
