...
| 実装コスト | 大（tape スキーマ + 記録 + エンジン + 台帳 + fill model + agentKernel 抽出） | 小（regime config + CLI + state 配布。bot.ts 無改修） |
...
|------|----------------------------------|--------------------------------------|
| 実装 | 既存 sim:realtime そのまま | mine トリガー（agent tx 到着 or タイムアウト）の追加 |
| 再現性 | 統計的（tx タイミング非決定。ADR 0005 と同性質） | rule agent なら実質ビット再現（LLM の非決定性のみ残る） |
...
- 新規コードは **backtest CLI（薄いラッパ）+ B2 ステップモード + state dump 生成スクリプト**のみ。
  coordinator・sdk・runtime の既存契約は一切変えない。依存方向 `example → sdk ← core` にも変更なし
...
**B1: 実時間再生（本線）** — 既存 sim:realtime をそのまま使う。ローカルデプロイは anvil 実行が
ミリ秒オーダーなので、`blockTimeSec` を短縮（例 0.5 秒）して実時間の数倍で回せる。
tx タイミングの非決定性は残るが、これは本番 realtime と同じ性質であり、
「同一 regime を N 回回して分布を見る」統計評価にはむしろ正しい挙動である。

**B2: 同期ステップ再生（回帰テスト・paired 比較用）** — 壁時計から切り離したターン制で進める:
毎ブロック、flow 注文の投入後に **agent の tx が mempool に到着するかタイムアウトした時点で mine** し、
次ブロックへ進む。agent 契約（bot.ts / env）は無変更で、coordinator 側の mine トリガーだけを変える。
rule agent なら初期状態・seed・順序が全て固定され**実質ビット再現**になり、
「コードを 1 行変えたら結果がどう変わるか」の paired 比較ができる（prompt 型は LLM 自体の
非決定性が残る）。realtime とタイミング意味論が異なるため、B2 の結果は回帰・比較専用とし
最終確認は B1 で行う。
...
- bot.ts / sdk / coordinator が無改修 = バックテストと realtime のランタイム乖離というリスク自体が生まれない
...
- 純 TS replay（旧 A 案）の秒単位速度・完全決定論は得られない
  - → B2 ステップ駆動 + blockTimeSec 短縮 + ポート別並列で実用域へ。rule agent の回帰は B2 が
    実質ビット再現を担う
...
| B2 の mine トリガー詳細（tx 到着検知とタイムアウトの較正） | 実装しないと決められない | Phase 1 実装時 |
...
1. **Phase 0**: state dump 生成スクリプト + regime config 数本（calm / crash）+ backtest CLI（B1、
   anvil ライフサイクル・`--repeat` 内包）
2. **Phase 1**: B2 同期ステップモード（coordinator の mine トリガー）+ equity curve 出力の整備 + 較正
   （同一 regime での B1/B2/realtime 比較）
