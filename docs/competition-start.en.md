...
These are the eight kinds rules §3.2 publishes plus `spike` (issue #105) and `depeg-persist` (issue #106); the rules' list needs both additions. Which epoch is which regime is never announced, but
**the kinds themselves and their generators are public**: `config/regimes/<name>.yaml`. The public set
`config/scenarios/public.yaml` is 10 regimes × 5 seeds = 50 scenarios; the non-public set is drawn from
