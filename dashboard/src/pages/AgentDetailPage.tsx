...
function PositionRow({
  ...
  const sideColor = position.side === "long" ? "var(--success-text)" : "var(--danger-text)";
  const pnlColor = position.pnlPercent >= 0 ? "var(--success-text)" : "var(--danger-text)";
  return (
    <div
      style={{
        display: "grid",
    ...
    >
      ...
      <span style={{ color: sideColor, textTransform: "uppercase" }}>{position.side}</span>
...
function TradeRow({ trade }: { trade: AgentTrade }) {
  return (
    ...
    >
      ...
      <span style={{ textAlign: "right", color: "var(--text-tertiary)" }}>{trade.time}</span>
    </div>
  );
}
    ...
    <div style={{ minHeight: "100vh", display: "flex", background: "var(--bg-canvas)" }}>
      ...
      >
        <span
          ...
          style={{ font: "var(--text-sm) var(--font-mono)", color: "var(--text-link)", textDecoration: "none", cursor: "pointer" }}
        >
        ...
        >
          ...
          <div>
            <div style={{ font: "var(--weight-semibold) var(--text-md) var(--font-sans)", color: "var(--text-primary)" }}>
              {agent.agent}
            </div>
            <div style={{ font: "var(--text-xs) var(--font-mono)", color: "var(--text-tertiary)" }}>
              {agent.address} — {agent.strategy}
            </div>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "16px" }}>
          ...
          <StatCard label="PnL" value={`${agent.pnlPercent >= 0 ? "+" : ""}${agent.pnlPercent.toFixed(1)}%`} tone="success" delta={`${agent.pnlPercent >= 0 ? "+" : ""}${agent.pnlPercent.toFixed(1)}%`} />
          <StatCard label="Sharpe" value={agent.sharpe.toFixed(2)} />
          <StatCard label="Max drawdown" value={`${agent.maxDrawdownPercent.toFixed(1)}%`} tone="danger" delta={`${agent.maxDrawdownPercent.toFixed(1)}%`} />
        </div>
        ...
        {tab === "overview" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "20px" }}>
            ...
            >
              ...
              <Sparkline points={agent.portfolioPoints} tone="success" width={500} height={140} />
              <span style={{ ...SECTION_LABEL_STYLE, marginTop: "6px" }}>Open positions</span>
              <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                ...
                {agent.positions.map((p, i) => (
                  <PositionRow key={i} position={p} grid={POSITIONS_GRID} padding="7px 2px" />
                ))}
              </div>
            </div>
            <div
            ...
            >
              <span style={{ ...SECTION_LABEL_STYLE, display: "block", marginBottom: "10px" }}>Decision log — live ↓</span>
        ...
        {tab === "positions" && (
          <div style={{ background: "var(--bg-surface-raised)", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-lg)", overflow: "hidden" }}>
            <div
              style={{
                display: "grid",
            ...
            {agent.positions.map((p, i) => (
              <PositionRow key={i} position={p} grid={positionsGridWithHeader} padding="12px 16px" borderSide="borderBottom" />
            ))}
          </div>
        )}
