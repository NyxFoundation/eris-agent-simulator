...
const RANK_RING: Record<number, { bg: string; fg: string }> = {
  1: { bg: "color-mix(in oklch, var(--warning) 16%, white)", fg: "var(--warning)" },
...
function PodiumCard({ entry }: { entry: ArchivePodiumEntry }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ font: "var(--text-sm) var(--font-mono)", color: "var(--success-text)" }}>
  ...
  return (
    ...
    >
      <span style={{ color: "var(--text-tertiary)" }}>
        {isTop3 ? <CrownIcon color={RANK_RING[row.rank]?.fg ?? RANK_RING[3].fg} /> : row.rank}
      </span>
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", background: "var(--bg-canvas)" }}>
      ...
      <div style={{ padding: "24px 32px", borderBottom: "1px solid var(--border-subtle)", background: "var(--bg-surface)" }}>
        <div style={{ ...SECTION_LABEL_STYLE, marginBottom: "14px", textAlign: "center" }}>Round winners</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "16px", maxWidth: "900px", margin: "0 auto" }}>
          <StatCard label="Total tx" value={stats.totalTx.toLocaleString("en-US")} />
          <StatCard label="Agents entered" value={stats.agentsEntered} />
          <StatCard label="Total volume" value={stats.totalVolume} />
          <StatCard label="Liquidations" value={stats.liquidations} tone="danger" />
        </div>
      </div>

      <main style={{ display: "grid", gridTemplateColumns: "1fr 280px", flex: 1, width: "100%", boxSizing: "border-box" }}>
        <div style={{ padding: "20px 32px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
            <span style={SECTION_LABEL_STYLE}>Final leaderboard</span>
            <span
              onClick={() => navigate("/leaderboard")}
              style={{ font: "var(--text-sm) var(--font-sans)", color: "var(--text-link)", textDecoration: "none", cursor: "pointer" }}
            >
        ...
        >
          <div>
            <div style={{ ...SECTION_LABEL_STYLE, marginBottom: "8px" }}>Closing prices</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              {closingPrices.map((m) => (
                <div key={m.pair} style={{ display: "flex", justifyContent: "space-between", font: "var(--text-sm) var(--font-mono)" }}>
                  <span style={{ color: "var(--text-secondary)" }}>{m.pair}</span>
                  <span style={{ color: "var(--text-primary)" }}>{m.price.toLocaleString("en-US")}</span>
                </div>
              ))}
            </div>
          </div>
          <div>
            <div style={{ ...SECTION_LABEL_STYLE, marginBottom: "8px" }}>Events injected</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              {events.map((e, i) => (
                <div key={i} style={{ font: "var(--text-xs) var(--font-mono)", color: "var(--text-secondary)" }}>
                  {e.time} — {e.text}
                </div>
              ))}
            </div>
