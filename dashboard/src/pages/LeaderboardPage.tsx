...
function LeaderboardTableRow({ row }: { row: AgentStanding }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ color: "var(--text-secondary)", fontFamily: "var(--font-sans)" }}>{row.strategy}</span>
      <span style={{ textAlign: "right", color: "var(--text-primary)" }}>{row.score.toFixed(1)}</span>
      <span style={{ textAlign: "right", color: pnlColor }}>
        {row.pnlPercent >= 0 ? "+" : ""}
        {row.pnlPercent.toFixed(1)}%
      </span>
      <span style={{ textAlign: "right", color: "var(--text-primary)" }}>{row.sharpe.toFixed(2)}</span>
      <span style={{ textAlign: "right", color: "var(--danger-text)" }}>{row.maxDrawdownPercent.toFixed(1)}%</span>
    </div>
  );
}
...
export function LeaderboardPage() {
  ...
  const [strategyFilter, setStrategyFilter] = useState<"all" | StrategyCategory>("all");
  ...
  const filteredRows = useMemo(() => {
    ...
    return data.leaderboard.filter((row) => {
      const matchesTerm =
        !term || row.agent.toLowerCase().includes(term) || row.strategy.toLowerCase().includes(term);
      const matchesStrategy = strategyFilter === "all" || row.strategyCategory === strategyFilter;
      ...
      >
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: "14px" }}>
            <span style={{ font: "var(--weight-bold) var(--text-xl) var(--font-sans)", color: "var(--text-primary)" }}>
              Leaderboard
            </span>
            <span style={{ font: "var(--text-sm) var(--font-sans)", color: "var(--text-tertiary)" }}>
              Season 3 · DeFi Strategy Open
            </span>
          </div>
          <span style={{ font: "var(--text-xs) var(--font-mono)", color: "var(--text-tertiary)" }}>
            score = blended PnL / risk-adjusted return / drawdown
          </span>
        </div>

        <div style={{ display: "flex", gap: "12px", alignItems: "flex-end" }}>
          <div style={{ flex: 1, minWidth: "220px" }}>
            <Input placeholder="Search agent…" mono value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <div style={{ minWidth: "200px" }}>
            <Select
              value={strategyFilter}
              options={STRATEGY_OPTIONS}
              onChange={(e) => setStrategyFilter(e.target.value as "all" | StrategyCategory)}
        ...
        >
          ...
          {filteredRows.length === 0 && (
            <div style={{ padding: "18px 16px", font: "var(--text-sm) var(--font-sans)", color: "var(--text-tertiary)" }}>
