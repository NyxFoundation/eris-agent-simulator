...
//
// The part worth pinning is the seam. An interval boundary belongs to the segment it falls in, but
// the interval into a segment's first boundary began at the last boundary of the one before — so a
// slice that starts strictly at the segment loses one interval per segment, seven over a week, and
...
import { SegmentedRun, sliceIntervalSeries } from "../core/src/segments.js";
...
test("a segment starting mid-interval carries the boundary before it", () => {
  // The roll landed at 125, between boundaries. The interval that ends at 130 began at 120, outside
  // this segment — without carrying 120 the segment would open with no value to return from, and
  // that interval would belong to nobody.
  const cut = sliceIntervalSeries(series, 125, 140);
...
test("a segment starting on a boundary carries nothing extra", () => {
  ...
  // segment's last return to this one as well — the same interval counted twice.
  const cut = sliceIntervalSeries(series, 120, 140);
  assert.deepEqual(cut.boundaryBlocks, [120, 130, 140]);
  assert.deepEqual(cut.valuesByAgent.a, [3, 4, 5]);
});

test("the first segment has no boundary before it and does not invent one", () => {
  const cut = sliceIntervalSeries(series, 100, 120);
  assert.deepEqual(cut.boundaryBlocks, [100, 110, 120]);
  assert.deepEqual(cut.valuesByAgent.b, [10, 20, 30]);
});

test("segments partition the intervals: every return belongs to exactly one", () => {
  // The seam is shared but the *returns* are not — which is the property that matters, because the
  // score averages returns.
  const first = sliceIntervalSeries(series, 100, 120);
  const second = sliceIntervalSeries(series, 120, 140);
...
test("a segment holding no boundary yields no interval rather than a phantom one", () => {
  const cut = sliceIntervalSeries(series, 141, 150);
