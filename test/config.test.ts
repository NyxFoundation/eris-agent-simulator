...
test("ADR 0015 §6: a roster omitting command passes as-is, relying on convention resolution", () => {
  const path = writeAgentsFile({
    agents: [
...
test("ADR 0015 §6: specifying args alone (without command) is rejected", () => {
  const path = writeAgentsFile({
