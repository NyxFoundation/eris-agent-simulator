...
test("validateAction: buy->sell bundle passes even under USDC-only (ADR 0011 swap output credit)", () => {
  // wethWei=0 (USDC-only funding zeroes initial β). The buy leg (USDC->WETH) credits its output WETH so
  // the sell leg (WETH->USDC) is not rejected at zero balance (lets us measure pure α).
...
test("validateAction: buy->sell bundle passes even with balances.bases (work.bases sync, ADR 0013)", () => {
  // direct-mode getBalances returns bases={WETH:wethWei,...}. In that case adapter.validate draws the
  // balance from bases[base], so unless the buy leg's output WETH is added to bases["WETH"] via creditBase,
  // the sell leg is rejected at zero balance. Regression test that bases stays in sync.
...
test("validateAction: a standalone WETH sell under USDC-only is rejected for insufficient balance (no credit source)", () => {
  const usdcOnly: BalanceSnapshot = {
    ethWei: 1n,
    wethWei: 0n,
    usdcUnits: 100n,
...
test("validateAction enforces cumulative balance across a bundle of LP mints", () => {
  // balances.wethWei = 100. A 2-item bundle where each mint requests 100 WETH should be
  // rejected on the second item because the cumulative amount exceeds balance (each would pass standalone).
  const action = parseAction({
    type: "bundle",
    actions: [
      {
