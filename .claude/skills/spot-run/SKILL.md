...
config が不明なら `ls eris.config.*.yaml` を見て提案（追跡対象は `eris.config.all18-mixed.yaml`＝36体 multi-venue / `eris.config.claude-llm.yaml`＝LLM の 2 つ）。各 config はロスターを inline `agents:` で内包する（run ノブも同居）。別ロスターを使うなら `--agents <path>`（JSON/YAML 可）。
