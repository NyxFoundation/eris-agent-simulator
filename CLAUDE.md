...
旧 `src/` / `examples/` は撤去済み。旧 LLM 自己改善機構（src/llm）と未参照戦略は `_archive/` に温存。
...
同梱の全 agent は agent.ts と併置で **prompt.md も持つ**（実行は agent.ts 優先 = ADR 0015 §2。
prompt.md は戦略の自然言語記述であり、LLM 駆動化・自己改善の起点。改善は prompt 本文を書き換える）。
